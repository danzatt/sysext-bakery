...
function _generate_signed_sysext() {
  ...
  if [[ -n "${PRE_SIGN_HOOK_SCRIPT:-}" ]] && [[ -x "$PRE_SIGN_HOOK_SCRIPT" ]]; then
          echo "Running pre-sign hook: $PRE_SIGN_HOOK_SCRIPT"
          "$PRE_SIGN_HOOK_SCRIPT"
  else
          echo "No pre-sign hook: $PRE_SIGN_HOOK_SCRIPT"
  fi

  CERT_CONTENT=$(env "${PKCS11_ENV[@]}" p11-kit \
      export-object "${PKCS_TOKEN_NAME};type=cert" -v)

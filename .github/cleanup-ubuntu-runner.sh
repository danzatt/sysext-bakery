...
#
# Taken from jlumbroso/free-disk-space GitHub Action

# ======
# MACROS
# ======

CLEANUP_ANDROID=true
CLEANUP_DOTNET=true
CLEANUP_HASKEL=true
# this takes too long
CLEANUP_LARGE_PACKAGES=false
CLEANUP_DOCKER_IMAGES=true
CLEANUP_TOOL_CACHE=true
CLEANUP_SWAP_STORAGE=true
...
if [[ $CLEANUP_LARGE_PACKAGES == 'true' ]]; then
  BEFORE=$(getAvailableSpace)
...
if [[ $CLEANUP_TOOL_CACHE == 'true' ]]; then
  BEFORE=$(getAvailableSpace)
...
if [[ $CLEANUP_SWAP_STORAGE == 'true' ]]; then
  BEFORE=$(getAvailableSpace)
